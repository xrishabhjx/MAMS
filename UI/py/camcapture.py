...
def take_attendance():
    ...
    while time.time() < end_time:
        ...
        if recognized_in_this_frame:
            recognized_count += 1
            print(f"Frame {frame_count}: Head Up (Recognized)")
        else:
            print(f"Frame {frame_count}: Head Down (Not recognized)")
    ...
    # Face score: ratio of frames where a known face was recognized
    face_score = 0
    if frame_count > 0:
        face_score = recognized_count / frame_count
    print(f"Recognized frames: {recognized_count} / {frame_count}")
